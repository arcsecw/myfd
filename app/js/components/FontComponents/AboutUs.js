import React from 'react';
var AboutUs = React.createClass({
    render() {
        return (
            <div>
                <link href="i/resources/css/axure_rp_page.css" type="text/css" rel="stylesheet"/>
                <link href="i/data/styles.css" type="text/css" rel="stylesheet"/>
                <link href="i/css/commonButtom/style.css" type="text/css" rel="stylesheet"/>
                <div id="11111" className="ax_paragraph">
                    <img id="u111_img" className="img " src="i/resources/images/transparent.gif"/>

                    <div id="1111" className="text">
                        <p><span>关于我们</span></p>
                    </div>
                </div>


                <div id="22222" className="ax_paragraph">
                    <img id="u113_img" className="img " src="i/resources/images/transparent.gif"/>

                    <div id="2222" className="text">
                        <p><span>联系我们</span></p>
                    </div>
                </div>


                <div id="33333" className="ax_paragraph">
                    <img id="u115_img" className="img " src="i/resources/images/transparent.gif"/>

                    <div id="u116" className="text">
                        <p><span>服务条款</span></p>
                    </div>
                </div>
                
                 <div id="u888" className="ax_paragraph">
            <img id="u125_img" className="img " src="i/resources/images/transparent.gif"/>

            <div id="u126" className="text">
              <p><span><img src = 'http://www.beian.gov.cn/img/ghs.png'/><a href = 'http://www.beian.gov.cn/portal/registerSystemInfo?recordcode=11010802022799'>京公网安备 11010802022799号</a></span></p>
             
            </div>
          </div>

                <div id="44444" className="ax_paragraph">
                    <img id="u117_img" className="img " src="i/resources/images/transparent.gif"/>

                    <div id="u118" className="text">
                        <p><span>媒体更新</span></p>
                    </div>
                    <div id="haha">
                        <input/>
                    </div>
                </div>
            </div>
        );
    }
});
export default AboutUs;
